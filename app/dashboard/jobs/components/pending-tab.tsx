      ...
      <div className="flex flex-col gap-8">
        <div className="flex gap-4 items-center flex-wrap">
          ...
          <button className="border border-neutral-500 rounded-md px-4 py-2 flex gap-4 items-center">
            <span className="text-base font-normal text-neutral-500">All categories</span>
            <ChevronDown className="w-8 h-8" />
          </button>

          <button className="border border-neutral-500 rounded-md px-4 py-2 flex gap-4 items-center">
            <span className="text-base font-normal text-neutral-500">All Budgets</span>
            <ChevronDown className="w-8 h-8" />
          </button>
          ...
          <button
              ...
              className="border border-neutral-500 rounded-md h-12 px-6 py-4 flex items-center gap-2"
          >
            <div
          ...
          <Button
              className="bg-primary-500 text-white px-7 py-3 rounded-md h-auto"
              onClick={handleBulkProcess}
              loading={bulkShortlist.isPending}
              disabled={selectedIds.length === 0 || bulkShortlist.isPending}
          >
...
function PendingJobCard({
      ...
      case "processing":
        return (
            <div className="bg-warning-50 px-2 py-2 rounded-[15px] h-8 flex items-center justify-center gap-2">
              ...
              <span className="text-sm font-normal text-warning-800">Processing</span>
            </div>
        );
      case "completed":
        return (
            <div className="bg-success-50 px-2 py-2 rounded-[15px] h-8 flex items-center justify-center gap-2">
              ...
              <span className="text-sm font-normal text-success-800">Completed</span>
            </div>
        );
      case "failed":
        return (
      ...
      default:
        return (
            <div className="bg-success-50 px-2 py-2 rounded-[15px] h-8 flex items-center justify-center">
              <span className="text-sm font-normal text-success-800">Ready for review</span>
            </div>
        );
    }
  };
  ...
  return (
      <>
        <div
            className={`border rounded-md p-6 flex flex-col gap-4 transition-colors ${
                isSelected ? "border-primary-500 bg-primary-50" : "border-border-500"
            }`}
        >
          <div className="flex gap-4 items-start justify-between">
            ...
            <div className="flex flex-col gap-2 flex-1">
              ...
              <div className="flex flex-col gap-4">
                <p className="text-base font-normal text-secondary-500">{job.companyName}</p>
                <p className="text-xs font-normal text-neutral-500">ID: {job.jobId}</p>
              </div>
            </div>
            {getStatusBadge()}
          </div>
          ...
          <div className="flex gap-20">
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className={`text-sm font-normal tracking-[0.1px] ${getUrgencyColor(job.urgency)}`}>
                {job.urgency}
              </p>
            </div>
          </div>
